...
if(isset($_POST['saveO'])){
      $time=$_POST['time'];
      $day=$_POST['day'];
      $query="INSERT INTO  opening_hours(time,day)
            VALUES('$time','$day')";
      $result= mysqli_query($conn,$query);}
      
if(isset($_GET['delete'])){ 
      $id=$_GET['delete'];
      $delQuery="DELETE FROM opening_hours WHERE id=$id";
      $delResult= mysqli_query($conn,$delQuery);
}
if(isset($_GET['edit'])){
      $id=$_GET['edit'];
      $update=true;
       $editQuery="SELECT * FROM opening_hours WHERE id=$id";
...
if(isset($_POST['editN'])){
      ...
      $time=$_POST['time'];
      $day=$_POST['day'];
      $editQuery="UPDATE opening_hours SET time='$time', day='$day' WHERE id='$id'";
      $editResult= mysqli_query($conn,$editQuery);
} 
...
if(isset($_POST['save'])){
      ...
      $query="INSERT INTO  contact_info(phone_number,email,address)
            VALUES('$phone_number','$email','$address')";
      $result= mysqli_query($conn,$query);}
      
if(isset($_GET['delete'])){ 
      $id=$_GET['delete'];
      $delQuery="DELETE FROM contact_info WHERE id=$id";
      $delResult= mysqli_query($conn,$delQuery);
}
if(isset($_GET['edit'])){
      $id=$_GET['edit'];
      $update=true;
       $editQuery="SELECT * FROM contact_info WHERE id=$id";
      $editResult=mysqli_query($conn,$editQuery);
          if(mysqli_num_rows($editResult) != 0){
            $row=$editResult->fetch_array();
...
if(isset($_POST['editN'])){
      ...
      $email=$_POST['email'];
      $address =$_POST['address'];
      $editQuery="UPDATE contact_info SET phone_number='$phone_number', email='$email', address='$address' WHERE id='$id'";
      $editResult= mysqli_query($conn,$editQuery);
} 
...
<head>
    <meta charset="utf-8">
    <title>Clever Mind POB - Admin Template</title>
                ...
                <div class="row g-4">
                    <div class="col-sm-12 col-xl-12">
                        <div class="bg-secondary rounded h-100 p-4">
                            ...
                            <form action="contact_add_info.php" method="post">
                        <div class="form-group">
                            <label for="phone_number">Phone number</label>
                            <input type="text" value="<?php echo $row['phone_number'] ?? '' ?>" name="phone_number" required class="form-control" id="phone_number">
                        </div>
                        <div class="form-group">
                            <label for="email">Email</label>
                            <input type="text" value="<?php echo $row['email'] ?? '' ?>" name="email" required class="form-control" id="email">
                        </div>
                        <div class="form-group">
                            <label for="address">insta link</label>
                            <input type="text" value="<?php echo $row['address'] ?? '' ?>" name="address" required class="form-control" id="address">
                        </div>
                    <div class="btns">
                                          <?php
                                                if($update==true): ?>
                                          <a href="contact_view_info.php"><input class="btn btn-primary mr-2" type="submit"
                                                    name="editN" value="Edit"> </a>
                                          <?php else: ?>
                                          <a href="contact_add_info.php"><input class="btn btn-primary mr-2" type="submit"
                                                    name="save" value="Save"> </a>
                                          <?php endif; ?>
                                          <button class="btn btn-light">Cancel</button>
                                          <a href="acontact_view_info.php" class="btn btn-primary mr-2">View all</a>
                                    </div>
                    </form>
                ...
                <div class="row g-4">
                    <div class="col-sm-12 col-xl-12">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" value="<?php echo $row['time'] ?? '' ?>" name="time" required class="form-control" id="time">
                        </div>
                        <div class="form-group">
                            <label for="text">Day</label>
                            <input type="day" value="<?php echo $row['day'] ?? '' ?>" name="day" required class="form-control" id="day">
                        </div>
                    <div class="btns">
                                          <?php
                                                if($update==true): ?>
